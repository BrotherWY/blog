        ...
        { title: '当前版本', dataIndex: 'version', key: 'version' },
        {
          ...
          render: (text, record) => (
            <span>
              ...
              <Popconfirm title="确定发布吗?" onConfirm={() => { this.handleDelete(record.id); }} okText="yes" cancelText="no">
                <span style={{ color: '#4396ec', cursor: 'pointer' }}>发布</span>
              </Popconfirm>
            </span>
          ),
        },
    ...
    return (
      <div>
        ...
        <Table
          ...
          bordered
          footer={() => (<Button type="primary" icon="delete" onClick={this.handleBatchDel}>批量发布</Button>)}
          rowSelection={{
