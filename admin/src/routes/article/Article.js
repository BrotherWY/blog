...
class Article extends Component {
        ...
        { title: '当前版本', dataIndex: 'version', key: 'version' },
        {
          ...
          render: (text, record) => (
            <span>
              ...
              <Popconfirm title="确定下架吗?" onConfirm={() => { this.handleDelete(record.id); }} okText="yes" cancelText="no">
                <span style={{ color: '#4396ec', cursor: 'pointer' }}>下架</span>
              </Popconfirm>
            </span>
          ),
        },
  ...
  handleView(record) {
    // 前往独立页面查看
    this.props.history.push('/article/view', {
      article: record,
    });
  }
    ...
    return (
      <div>
        ...
        <Table
          ...
          bordered
          footer={() => (<Button type="primary" icon="delete" onClick={this.handleBatchDel}>批量下架</Button>)}
          rowSelection={{
